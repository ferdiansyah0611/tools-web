...
const configure = {
  list: {
    template: ["vanilla", "vue", "react", "preact", "lit", "svelte", "solid", "qwik"],
  },
};
...
vite
  ...
  .addOption(new Option("--template <name>", "template name").choices(configure.list.template).makeOptionMandatory())
  .option("--ts", "enable typescript")
  .action(makeProject);

export async function makeProject(name: string, option: any) {
  const task = new Task(["Read Configuration", "Checking NPM", "Execution", "Generate Code"]);
  task.start(0);
  ...
  if (value.mode === 1) {
    ...
    if (option.template === "react") {
      execution += " && npm i @reduxjs/toolkit react-redux react-router-dom axios";
    }
  }
  ...
  if (option.template === "react") {
    ...
    file.copy(paths.data.react + "route/index.jsx", paths.directory.route(["index.jsx"], dir));
    file.copy(paths.data.react + "route/Home.jsx", paths.directory.route(["Home.jsx"], dir));
    file.copy(paths.data.react + "route/About.jsx", paths.directory.route(["About.jsx"], dir));
    file.copy(paths.data.react + "service/auth.js", paths.directory.service(["auth.js"], dir));
    file.copy(paths.data.react + "service/http.js", paths.directory.service(["http.js"], dir));
    file.copy(paths.data.react + "store/index.js", paths.directory.store(["index.js"], dir));
    file.copy(paths.data.react + "store/app.js", paths.directory.store(["app.js"], dir));
    file.copy(paths.data.react + "component/template.jsx", paths.directory.component(["template.jsx"], dir));
    file.copy(paths.data.react + "App.jsx", paths.directory.src(["App.jsx"], dir));
    file.copy(paths.data.react + "main.jsx", paths.directory.src(["main.jsx"], dir));
  } else if (option.template === "vue") {
    file.mkdir(paths.directory.route([], dir));
    file.mkdir(paths.directory.store([], dir));
    file.copy(paths.data.vue + "router.js", paths.directory.route(["index.js"], dir));
    file.copy(paths.data.vue + "Home.vue", paths.directory.route(["Home.vue"], dir));
    file.copy(paths.data.vue + "storeindex.js", paths.directory.store(["index.js"], dir));
    file.copy(paths.data.vue + "App.vue", paths.directory.src(["App.vue"], dir));
    file.copy(paths.data.vue + "main.js", paths.directory.src(["main.js"], dir));
  }
  // update file
  ...
  file.write(dir + "/vercel.json", '{ "routes": [{ "src": "/[^.]+", "dest": "/", "status": 200 }] }');
  task.success(3);
}
...
export async function addTailwind(option: any) {
  ...
  file.copy(paths.data.tailwind + "tailwind.config.cjs", dir + "/tailwind.config.cjs");
  task.success(2);
}