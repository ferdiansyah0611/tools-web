...
export function compactName(name: string, format: string): CompactNameReturn {
	...
	return {
		...
		get titleCaseWordOnly() {
			if (!data.titleCaseWordOnly) data.titleCaseWordOnly = toTitleCase(this.Var).replaceAll(/[-_]+/g, "");
		...
		get pathCamelCase() {
			if (!data.pathCamelCase) data.pathCamelCase = toAddFormat(this.folder + "/" + this.camelCase, format);
			return data.pathCamelCase;
		},
		get pathTitleCase() {
			if (!data.pathTitleCase) data.pathTitleCase = toAddFormat(this.folder + "/" + this.titleCase, format);
		...
		get pathTitleCaseNoSeparate() {
			if (!data.pathTitleCaseNoSeparate) {
				...
				if(lastSlash !== -1) {
					v = v.slice(0, lastSlash) + v.slice(lastSlash).replaceAll(/[-_]+/g, "");
				}
