...
export type Result = {
   ...
   */
  doRun: () => ChildProcessWithoutNullStreams;
  /**
...
export function execute(cli: string, option: Option): Result {
  ...
  function doRun() {
    ...
    let spawnOptions: SpawnOptionsWithoutStdio = {
      shell: true,
    ...
    running.stdin.on("data", option.stdin ? option.stdin : callback);
  ...
  function doSync() {
    let closed = beforeStart();
    let running: SpawnSyncReturns<Buffer> = spawnSync(cli, {
      shell: true,
      signal: controller.signal,
    });
    // after run
...
export function prettier(dir: string, target: string) {
  return execute(
    `cd ${dir} && npx prettier ${target} --write --log-level silent`,
    {},
  ).doSync();
}
