...
var user, token = () => ({'x-access-token': user.data.token})

describe('Testing Page', () => {
    before((done) => {
        const signup = () =>  http.post('/api/auth/signup').send({
            name: 'admin',
            email: 'admin@gmail.com',
            password: 'helloworld'
        })
        const signin = () => http.post('/api/auth/signin').send({
            email: 'admin@gmail.com',
            password: 'helloworld',
        }).then((response) => {
            user = response._body
    ...
    describe('data example', () => {
        it("add /api/example", (done) => {
            http.post('/api/example').set(token()).send({
                name: 'test',
                description: 'hi!'
            }).expect(200, done)
        })
        it("patch /api/example", (done) => {
            http.patch('/api/example/1').set(token()).send({
                name: 'test',
                description: 'hi!'
            }).expect(200, done)
        })
