...
import { getStorage, ref, uploadBytes, deleteObject, getDownloadURL } from "https://www.gstatic.com/firebasejs/9.6.1/firebase-storage.js"
import { 
  getAuth,
...
import {
  getFirestore,
  collection, addDoc, getDocs, updateDoc, deleteDoc, getDoc, setDoc,
  serverTimestamp,
  query, orderBy, startAfter, limit, doc, where, onSnapshot
...
export const registered = (email, password) => createUserWithEmailAndPassword(auth, email, password)
export const signed = (email, password) => signInWithEmailAndPassword(auth, email, password)
...
export const addData = async(table, data) => addDoc(collection(db, table), data)
export const setData = async(table, id, data) => setDoc(doc(db, table, id), data)
export const getData = (table) => getDocs(collection(db, table))
export const getDocsData = (table, id) => getDoc(doc(db, table, id))
export const updateData = (table, id, data) => updateDoc(doc(db, table, id), data)
...
export const paginateData = (table, limit, query, last = false) => {
  if(!last){
    ...
    const next = query(collection(db, table),
        orderBy("population"),
