...
export default function caseName() {
  ...
  const edit = useCallback(
    (id) => () => setstate(STORE.data.find((v) => v.id === id)),
    [STORE.data]
  );
      ...
      <div>
        ...
        <div className="overflow-auto">
          <table>
            <thead>
              <tr>
                <th className="cursor-pointer asc" onClick={sort("id")}>
                  ID
                </th>
                {/*table*/}
            ...
            <tbody>
              {STORE.paginate.map((v, i) => (
                <tr key={i}>
                  <td>{v.id}</td>
                  <td>
                    <button className="primary" onClick={edit(v.id)}>
                      Edit
                    </button>
                    <button className="ml-2 danger" onClick={deleted(v.id)}>
                      delete
                    </button>
                  </td>
                </tr>
        ...
        <div className="footer">
          ...
          <div className="btn">
            <button className="primary" onClick={previous}>
              Previous
            </button>
            {Array.from(new Array(allnumber)).map((v, i) => (
              <button
                key={i}
                className={page.offset === i + 1 ? "primary active" : "primary"}
